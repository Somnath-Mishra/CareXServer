...
export const createSchedule = asyncHandler(async (req, res) => {
    ...
    if (!startTime || !endTime) {
        throw new ApiError(
            400,
            "Start and end time required for creating schdule"
        );
    }
    if (mode?.trim() !== "" && location?.trim() === "") {
        throw new ApiError(
            400,
            "Location required for creating schdule in offline"
        );
    }
    if (mode?.trim() === "") {
...
export const createScheduleAutomatically = async (
    startTime,
    endTime,
    mode,
    location,
...
export const updateSchedule = asyncHandler(async (req, res) => {
    ...
    if (!scheduleId || !startTime || !endTime) {
        throw new ApiError(
            400,
            "Schedule id, start and end time required for updating schedule"
        );
    }
    ...
    if (mode?.trim() !== "" && location?.trim() === "") {
        throw new ApiError(
            400,
            "Location required for updating schdule for offline"
        );
    }
    if (mode?.trim() === "") {
        //TODO: call google meet api and create an link for meeting
    ...
    if (!updateSchedule) {
        throw new ApiError(
            500,
            "Something went wrong while updating schedule in database"
        );
    }
    res.status(200).json(
...
export const isScheduleFull = asyncHandler(async (req, res) => {
    ...
    if (!scheduleId) {
        throw new ApiError(
            400,
            "Schedule id is required for checking wheather the schedule is full"
        );
    }
    ...
    if (!slotInd) {
        res.status(200).json(new ApiResponse(200, true, "Schedule is full"));
    }
    res.status(200).json(new ApiResponse(200, false, "Schedule is not full"));
});
